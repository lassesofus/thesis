...
OUTPUT_DIR = Path("/home/s185927/thesis/experiments/01_zero_shot_single_axis_reaching/follow_up_experiments/latent_distribution_comparison")
...
def resize_to_square(frame: np.ndarray, size: int = 256) -> np.ndarray:
    """
    Center-crop and resize frame to a square.
...
def create_frame_collage(
    ...
    """
    Create a collage of sampled frames for thesis appendix.

    Creates a figure with DROID frames on top and simulation frames on bottom.

    Args:
        droid_frames: List of DROID RGB frames
        sim_frames: List of simulation RGB frames
        output_dir: Directory to save the collage
        n_cols: Number of columns in the grid
        n_rows: Number of rows per domain
        seed: Random seed for selecting frames to display
        frame_size: Size to resize frames to (square)
    """
    rng = np.random.default_rng(seed)

    n_frames_per_domain = n_cols * n_rows

    # Randomly select frames to display
    droid_indices = rng.choice(len(droid_frames), size=min(n_frames_per_domain, len(droid_frames)), replace=False)
    sim_indices = rng.choice(len(sim_frames), size=min(n_frames_per_domain, len(sim_frames)), replace=False)

    # Resize selected frames to squares
    selected_droid = [resize_to_square(droid_frames[i], frame_size) for i in droid_indices]
    selected_sim = [resize_to_square(sim_frames[i], frame_size) for i in sim_indices]

    # Create figure with GridSpec for better control over spacing
    fig = plt.figure(figsize=(n_cols * 2, n_rows * 2 * 2 + 1))

    # Create two separate GridSpecs for DROID and Simulation with gap between them
    gs_droid = fig.add_gridspec(
        n_rows, n_cols,
    ...
    # Plot DROID frames (top half)
    for i, frame in enumerate(selected_droid):
        row = i // n_cols
        col = i % n_cols
        if row < n_rows:
            ax = fig.add_subplot(gs_droid[row, col])
            ax.imshow(frame)
            ax.set_aspect('equal')
            ax.axis("off")

    # Plot simulation frames (bottom half)
    for i, frame in enumerate(selected_sim):
        row = i // n_cols
        col = i % n_cols
        if row < n_rows:
            ax = fig.add_subplot(gs_sim[row, col])
            ax.imshow(frame)
            ax.set_aspect('equal')
            ax.axis("off")

    # Add domain labels - positioned just above each grid
    fig.text(0.5, 0.97, "DROID (Real)", ha="center", va="bottom",
             fontsize=PLOT_PARAMS["subtitle_size"], fontweight="bold")
    fig.text(0.5, 0.48, "RoboHive (Simulated)", ha="center", va="bottom",
             fontsize=PLOT_PARAMS["subtitle_size"], fontweight="bold")

    plt.savefig(output_dir / "frame_collage.png", dpi=200, bbox_inches="tight",
                facecolor="white", edgecolor="none")
    plt.savefig(output_dir / "frame_collage.pdf", bbox_inches="tight",
                facecolor="white", edgecolor="none")
    plt.close()

    print(f"Saved frame collage to {output_dir / 'frame_collage.png'} and .pdf")
...
def main():
    ...
    parser.add_argument("--collage", action="store_true", help="Create frame collage for thesis appendix")
    parser.add_argument("--collage_cols", type=int, default=5, help="Number of columns in collage")
    parser.add_argument("--collage_rows", type=int, default=3, help="Number of rows per domain in collage")
    ...
    # Check if we need to get episodes
    need_episodes = args.collage or not (args.skip_encoding and cache_file.exists())
    ...
    if args.collage:
        n_collage_frames = args.collage_cols * args.collage_rows
        print(f"\nSampling {n_collage_frames} frames per domain for collage...")
        collage_droid_frames = sample_droid_frames(droid_episodes, n_collage_frames, args.seed + 100)
        collage_sim_frames = sample_sim_frames(sim_episodes, n_collage_frames, args.seed + 101)
        create_frame_collage(
            collage_droid_frames,
            collage_sim_frames,
            output_dir,
            n_cols=args.collage_cols,
            n_rows=args.collage_rows,
            seed=args.seed,
        )
