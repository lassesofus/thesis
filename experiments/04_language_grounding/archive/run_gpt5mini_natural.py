"""
Run gpt-5-mini with natural language prompt on diverse test pairs.
Creates PDF visualization of results.
"""
...
def generate_instructions(client, frame_k_path: str, frame_kd_path: str) -> dict:
    """Generate instructions for a frame pair."""
...
def create_pdf(results: list, output_path: str, base_dir: Path):
    ...
    with PdfPages(output_path) as pdf:
        for page in range(n_pages):
            ...
            fig = plt.figure(figsize=(16, 5 * len(page_results)))
            ...
            for i, item in enumerate(page_results):
                ...
                # Create subplots: [frame_k | frame_kd | instructions]
                gs = GridSpec(1, 3, figure=fig,
                              left=0.02, right=0.98,
                              top=1 - (i / len(page_results)) - 0.02,
                              bottom=1 - ((i + 1) / len(page_results)) + 0.02,
                              wspace=0.02, width_ratios=[1, 1, 1.5])
                ...
                for j, inst in enumerate(result.get("instructions", [])[:5], 1):
                    wrapped = textwrap.fill(f"{j}. {inst}", width=50)
    ...
    # Also save as PNG (first page preview)
    png_path = output_path.replace(".pdf", "_preview.png")

    fig = plt.figure(figsize=(16, 5 * min(3, len(results))))
...
def main():
    ...
    print(f"Processing {len(pairs)} pairs with gpt-5-mini...")
    ...
    json_path = output_dir / "gpt5mini_natural_results.json"
    ...
    # Create PDF
    pdf_path = str(output_dir / "gpt5mini_natural_instructions.pdf")
