#!/usr/bin/env python3
"""
Compare VJEPA2 models trained on different fractions of the DROID dataset.
...
MODELS = {
    "25%": BASE_DIR / "4.8.vitg16-256px-8f_025pct",
    "50%": BASE_DIR / "4.8.vitg16-256px-8f_050pct",
    "75%": BASE_DIR / "4.8.vitg16-256px-8f_075pct",
    "100%": BASE_DIR / "4.8.vitg16-256px-8f_100pct",
}
...
def plot_learning_curves(save_path=None):
    ...
    for (label, model_dir), color in zip(MODELS.items(), colors):
        ...
        # Mark the best epoch on validation plot
        best_idx = val_df['val_loss'].idxmin()
        best_epoch = val_df.loc[best_idx, 'epoch']
        best_val_loss = val_df.loc[best_idx, 'val_loss']
    ...
    # Add legend entry for best validation loss markers
    ax1.scatter([], [], s=150, color='gray', edgecolors='black', linewidths=2,
                label='Best Val Loss Epoch')
