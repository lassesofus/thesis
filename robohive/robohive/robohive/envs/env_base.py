...
class MujocoEnv(gym.Env, gym.utils.EzPickle, ObsVecDict):
    ...
    def examine_policy(self,
        ...
        # Disable any active debugger hooks (pdb/breakpoint/sys.settrace/sys.setprofile)
        try:
            import sys as _sys, os as _os, builtins as _builtins
            _os.environ["PYTHONBREAKPOINT"] = "0"
            if getattr(_sys, "gettrace", None) and _sys.gettrace() is not None:
                _sys.settrace(None)
            if hasattr(_sys, "setprofile"):
                _sys.setprofile(None)
            try: _sys.breakpointhook = lambda *a, **k: None
            except Exception: pass
            try: _builtins.breakpoint = lambda *a, **k: None
            except Exception: pass
        except Exception:
            pass
