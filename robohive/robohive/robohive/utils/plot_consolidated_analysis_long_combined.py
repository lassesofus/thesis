...
This version combines steps 1-5 from the 5-step experiment with steps 6-10 from the 10-step experiment.

Usage: python plot_consolidated_analysis_long_combined.py --out_dir /path/to/long_planning --short_dir /path/to/5step_experiment
"""
...
def load_direction_data_combined(long_dir, short_dir, direction, episode):
    """
    Load data for a single direction, combining steps 1-5 from short_dir with steps 6-10 from long_dir.
    """
    long_experiment_dir = os.path.join(long_dir, f"reach_along_{direction}")
    short_experiment_dir = os.path.join(short_dir, f"reach_along_{direction}")

    if not os.path.isdir(long_experiment_dir):
        print(f"Warning: Directory not found: {long_experiment_dir}")
        return None
    if not os.path.isdir(short_experiment_dir):
        print(f"Warning: Directory not found: {short_experiment_dir}")
        return None

    # Load long planning data (10 steps)
    long_data_path = find_data_file(long_experiment_dir)
    if not long_data_path:
        print(f"Warning: No data file found in {long_experiment_dir}")
        return None
    long_data = np.load(long_data_path, allow_pickle=True)

    # Load short planning data (5 steps)
    short_data_path = find_data_file(short_experiment_dir)
    if not short_data_path:
        print(f"Warning: No data file found in {short_experiment_dir}")
        return None
    short_data = np.load(short_data_path, allow_pickle=True)

    # Get per-episode arrays
    long_phase3_distances = long_data['phase3_distances_per_episode']
    short_phase3_distances = short_data['phase3_distances_per_episode']

    long_phase3_repr = long_data.get('phase3_repr_l1_distances_per_episode', None)
    short_phase3_repr = short_data.get('phase3_repr_l1_distances_per_episode', None)

    # Combine per-episode: take first 6 values from short (steps 0-5), then steps 6-10 from long
    # Note: phase3_distances has step 0 = starting distance, steps 1-N = after each planning step
    # For 5-step: indices 0-5 (6 values: start + 5 steps)
    # For 10-step: indices 0-10 (11 values: start + 10 steps)
    # We want: short[0:6] + long[6:11] = 11 values total

    combined_distances = []
    n_episodes = min(len(long_phase3_distances), len(short_phase3_distances))

    for i in range(n_episodes):
        short_ep = np.array(short_phase3_distances[i], dtype=float)
        long_ep = np.array(long_phase3_distances[i], dtype=float)

        # Take first 6 from short (step 0 through step 5)
        # Take remaining from long (step 6 through step 10)
        if len(short_ep) >= 6 and len(long_ep) >= 11:
            combined = np.concatenate([short_ep[:6], long_ep[6:11]])
            combined_distances.append(combined)
        else:
            print(f"Warning: Episode {i} has insufficient data (short={len(short_ep)}, long={len(long_ep)})")

    # Same for representation distances
    combined_repr = []
    if long_phase3_repr is not None and short_phase3_repr is not None:
        for i in range(n_episodes):
            short_ep = np.array(short_phase3_repr[i], dtype=float)
            long_ep = np.array(long_phase3_repr[i], dtype=float)

            if len(short_ep) >= 6 and len(long_ep) >= 11:
                combined = np.concatenate([short_ep[:6], long_ep[6:11]])
                combined_repr.append(combined)

    # Compute mean/std for combined distances
    dist_mean, dist_std, dist_len, dist_n = stack_episodes_to_mean_std(combined_distances)
    repr_mean, repr_std, repr_len, repr_n = (None, None, None, 0)
    if combined_repr:
        repr_mean, repr_std, repr_len, repr_n = stack_episodes_to_mean_std(combined_repr)

    # Load images from long planning directory
    start_img = None
    goal_img = None

    images_dir = find_planning_images_dir(long_experiment_dir, episode)
    if images_dir:
    ...
    if start_img is None:
        start_path = os.path.join(long_experiment_dir, f"run_{episode}_start.png")
        if os.path.exists(start_path):
            start_img = np.array(Image.open(start_path))

    if goal_img is None:
        goal_path = os.path.join(long_experiment_dir, f"run_{episode}_goal.png")
        if os.path.exists(goal_path):
            goal_img = np.array(Image.open(goal_path))
...
@click.command()
@click.option('--out_dir', type=str, required=True, help='Base directory for long planning (10-step) containing reach_along_{x,y,z} subdirectories')
@click.option('--short_dir', type=str, required=True, help='Base directory for short planning (5-step) containing reach_along_{x,y,z} subdirectories')
...
def main(out_dir, short_dir, episode, threshold):
    print(f"Loading long planning data from: {out_dir}")
    print(f"Loading short planning data from: {short_dir}")
    print(f"Episode (for images): {episode}")
    print(f"Combining steps 1-5 from short_dir with steps 6-10 from out_dir")

    # Load data for all directions
    all_data = {}
    for direction in DIRECTIONS:
        print(f"\nLoading {direction} direction...")
        data = load_direction_data_combined(out_dir, short_dir, direction, episode)
