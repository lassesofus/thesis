...
def main(experiment_dir, episode, grid_size):
    ...
    # Determine axis label format based on experiment type (k will be substituted per step)
    if experiment_type in ('x', 'y'):
        axis_label_fmt = (r'$\hat{{a}}_{{{k}}}^x$ (cm)', r'$\hat{{a}}_{{{k}}}^y$ (cm)')
    else:  # 'z'
        axis_label_fmt = (r'$\hat{{a}}_{{{k}}}^x$ (cm)', r'$\hat{{a}}_{{{k}}}^z$ (cm)')
    ...
    if has_position_errors:
        fig = plt.figure(figsize=(PLOT_PARAMS_LOCAL["figsize"][0], PLOT_PARAMS_LOCAL["figsize"][1] + 10))
        # Use nested gridspec: outer has 2 rows (top 2 rows together, bottom row separate)
        gs_outer = gridspec.GridSpec(2, 1, figure=fig, height_ratios=[2.1, 0.8],
                                      hspace=0.45, left=0.05, right=0.97, top=0.96, bottom=0.10)
        ...
        gs = gridspec.GridSpecFromSubplotSpec(2, NUM_FRAMES, subplot_spec=gs_outer[0],
                                               height_ratios=[1.0, 1.0], hspace=0.02, wspace=0.25)
        # Bottom row gridspec
        gs_bottom = gridspec.GridSpecFromSubplotSpec(1, NUM_FRAMES, subplot_spec=gs_outer[1], wspace=0.25)
    else:
    ...
    for step in range(NUM_FRAMES):
        ...
        # Generate step-specific axis labels
        axis_labels = (axis_label_fmt[0].format(k=step), axis_label_fmt[1].format(k=step))
    ...
    if last_im is not None:
        if has_position_errors:
            ...
            cbar_ax = fig.add_axes([0.05, error_pos.y1 + 0.09, 0.74, 0.02])  # [left, bottom, width, height]
        else:
    ...
    if has_position_errors:
        ...
        # Show 5cm success threshold only if within visible range
        y_min_val, y_max_val = min(position_errors_cm), max(position_errors_cm)
        y_pad = (y_max_val - y_min_val) * 0.3 if y_max_val > y_min_val else 2
        if 5 <= y_max_val + y_pad:
            ax_error.axhline(y=5, color='r', linestyle='--', linewidth=2.5, label='Success (5cm)')
        ...
        ax_error.tick_params(axis='both', labelsize=PLOT_PARAMS_LOCAL["tick_size"] + 10)
        ax_error.set_xticks(steps)
        # Auto-scale y-axis with padding to better show data variation
        y_min, y_max = min(position_errors_cm), max(position_errors_cm)
        y_padding = (y_max - y_min) * 0.3 if y_max > y_min else 2
        ax_error.set_ylim(bottom=max(0, y_min - y_padding), top=y_max + y_padding)
    ...
    legend_elements = [
        ...
        Line2D([0], [0], marker=r'$\rightarrow$', color='red',
               markersize=60, markeredgewidth=0, linestyle='None', label=r'Optimal $(\hat{a}_k^x, \hat{a}_k^y)$'),
        Line2D([0], [0], marker=r'$\rightarrow$', color='lime',
               markersize=60, markeredgewidth=0, linestyle='None', label=r'Planned $(\hat{a}_k^x, \hat{a}_k^y)$'),
    ]
    fig.legend(handles=legend_elements, loc='lower right',
               fontsize=PLOT_PARAMS_LOCAL["legend_fontsize"],
               bbox_to_anchor=(0.99, 0.32), frameon=False)
