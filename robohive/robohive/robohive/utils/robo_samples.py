...
# Energy metric module for metric sensitivity experiment
_metric_path = "/home/s185927/thesis/experiments/01_zero_shot_single_axis_reaching/follow_up_experiments/metric_sensitivity"
if os.path.isdir(_metric_path) and _metric_path not in sys.path:
    sys.path.insert(0, _metric_path)
try:
    from world_model_wrapper_metric import WorldModelWithMetric
    from energy_functions import EnergyFunction
except ImportError:
    WorldModelWithMetric = None
    EnergyFunction = None
...
@click.option(
    '--energy_metric',
    type=click.Choice(['l1', 'cosine', 'norm_l1']),
    default='l1',
    help='Energy function for CEM planning: l1 (baseline), cosine (scale-invariant), norm_l1 (DROID-normalized)'
)
@click.option(
    '--energy_stats_path',
    type=str,
    default='/home/s185927/thesis/experiments/01_zero_shot_single_axis_reaching/follow_up_experiments/metric_sensitivity/stats',
    help='Path to directory containing DROID statistics for norm_l1 metric (mu_droid.npy, cov_droid.npy)'
)
@click.option('--reset_sim', is_flag=True, default=False,
              help='Reset simulation state instead of physically returning home after Phase 1. '
                   'Ensures Phase 3 starts from exact home position.')
@click.option('--teleport_actions', is_flag=True, default=False,
              help='Teleport end-effector directly to target position instead of using trajectory execution. '
                   'Eliminates actuator/IK execution error for cleaner predictor bias measurement.')
...
def main(sim_path, horizon, radius, render, camera_name, out_dir, out_name,
         ...
         latent_alignment, latent_alignment_stats_path, energy_metric, energy_stats_path,
         reset_sim, teleport_actions):
    ...
    all_episode_data = {
        ...
        'phase3_predicted_energies': [],  # Predicted energy before action execution
    ...
    if enable_vjepa_planning:
        ...
        # Choose world model based on energy metric
        mpc_args = {
            "rollout": 1,
            "samples": 800,
            "topk": 10,
        ...
        if energy_metric != 'l1' and WorldModelWithMetric is not None:
            # Use WorldModelWithMetric for custom energy functions
            world_model = WorldModelWithMetric(
                encoder=encoder,
                predictor=predictor,
            ...
            print(f"V-JEPA models loaded with energy metric: {energy_metric}")
        else:
            # Use standard WorldModel for L1 baseline
            ...
            print(f"V-JEPA models loaded with baseline L1 energy.")

        # Initialize energy function for logging (for any metric)
        energy_fn_for_logging = None
        if EnergyFunction is not None:
            try:
                energy_fn_for_logging = EnergyFunction(
                    metric=energy_metric,
                    stats_path=energy_stats_path if energy_metric == 'norm_l1' else None,
                    device=str(device)
                )
                print(f"Energy function for logging: {energy_fn_for_logging}")
            except Exception as e:
                print(f"WARNING: Failed to initialize energy function for logging: {e}")
                energy_fn_for_logging = None
    ...
    while True:
        ...
        # Goal frame (captured via teleport before trajectory execution)
        goal_rgb = None
        ...
        while targets_reached < num_targets:
            ...
            all_episode_data['ik_success'].append(ik_result.success)

            # Capture goal frame at TRUE target position via teleport (not after trajectory execution)
            # This avoids the IK tracking error that occurs during physics simulation
            if targets_reached == num_targets - 1 and render == 'offscreen':
                # Save current state
                saved_qpos = sim.data.qpos[:ARM_nJnt].copy()
                saved_qvel = sim.data.qvel[:ARM_nJnt].copy()
                saved_ctrl = sim.data.ctrl[:ARM_nJnt].copy()

                # Teleport to IK solution (true goal position)
                sim.data.qpos[:ARM_nJnt] = ik_result.qpos[:ARM_nJnt]
                sim.data.qvel[:ARM_nJnt] = np.zeros(ARM_nJnt, dtype=float)
                sim.data.ctrl[:ARM_nJnt] = ik_result.qpos[:ARM_nJnt].copy()
                sim.forward()

                # Verify teleport accuracy
                teleport_ee_pos = sim.data.site_xpos[ee_sid].copy()
                teleport_error = np.linalg.norm(teleport_ee_pos - target_pos)
                print(f" Teleported to goal for frame capture (error: {teleport_error:.6f}m)")

                # Capture goal frame at true target position
                ...
                print(" Saved goal snapshot (at true target via teleport):", goal_path)

                # Skip trajectory execution - go directly to Phase 2/3
                # (goal frame already captured, no need to physically move there)
                targets_reached += 1
                continue

            # Only execute trajectory if we didn't capture via teleport
            ...
            # Execute trajectory to reach this target with distance tracking
            ...
            # Note: goal_rgb is now captured via teleport BEFORE trajectory execution
            # This fallback only runs if teleport capture didn't happen (e.g., non-offscreen mode)
            if targets_reached == num_targets:
                if goal_rgb is None and render == 'offscreen':
                    # Fallback: capture at current position (legacy behavior)
                    goal_rgb = sim.renderer.render_offscreen(
                        width=width,
                        height=height,
                        camera_id=camera_name,
                    ...
                    try:
                        ...
                        Image.fromarray(goal_rgb_resized).save(goal_path)
                        print(" Saved goal snapshot (fallback, at trajectory end):", goal_path)
                    except Exception as e:
                        print(" warning: could not save goal snapshot:", e)

                # For teleport mode, always capture frames; otherwise use frame_counter
                if goal_rgb is not None and (teleport_actions or (frame_counter[0] % steps_per_frame) == 0):
                    frames.append(goal_rgb)
                frame_counter[0] += 1

        # Phase 2: return to start
        if reset_sim:
            # Reset simulation state directly for exact home position
            print(f"\nResetting simulation to home position...")
            sim.data.qpos[:ARM_nJnt] = ARM_JNT0
            sim.data.qvel[:ARM_nJnt] = np.zeros(ARM_nJnt, dtype=float)
            sim.data.ctrl[:ARM_nJnt] = ARM_JNT0.copy()
            sim.forward()
            final_return_pos = ARM_JNT0.copy()
            print(f" Sim reset complete. EE at: {sim.data.site_xpos[ee_sid]}")
        else:
            # Physical return via min-jerk trajectory (record)
        ...
        # For teleport mode, always capture frames; otherwise use frame_counter
        if teleport_actions or (frame_counter[0] % steps_per_frame) == 0:
            frames.append(start_rgb)
        frame_counter[0] += 1

        # For teleport mode, hold at start position for visualization
        if teleport_actions and render == 'offscreen':
            start_qpos = sim.data.qpos[:ARM_nJnt].copy()
            num_steps = int(round(horizon / step_dt))
            for _ in range(num_steps):
                sim.data.qpos[:ARM_nJnt] = start_qpos
                sim.data.qvel[:ARM_nJnt] = 0.0
                sim.data.ctrl[:ARM_nJnt] = start_qpos.copy()
                sim.advance(render=False)
                if (frame_counter[0] % steps_per_frame) == 0:
                    rgb = sim.renderer.render_offscreen(
                        width=width, height=height,
                        camera_id=camera_name, device_id=device_id
                    )
                    if rgb is not None:
                        frames.append(rgb)
                frame_counter[0] += 1
        ...
        if (
            ...
            with torch.no_grad():
                ...
                for step_idx in range(planning_steps):
                    ...
                    # Calculate representation distance using configured energy metric
                    # (in aligned space if alignment is enabled)
                    if energy_fn_for_logging is not None:
                        repr_distance = energy_fn_for_logging(z_n, z_goal).item()
                    else:
                        repr_distance = torch.mean(torch.abs(z_n - z_goal)).item()
                    phase3_repr_l1_distances.append(repr_distance)
                    metric_str = f" (metric: {energy_metric})" if energy_metric != 'l1' else ""
                    align_str = f" (aligned: {latent_alignment})" if latent_alignment != 'none' else ""
                    print(f" Representation distance: {repr_distance:.6f}{metric_str}{align_str}")
                    ...
                    # Compute predicted energy for the selected action
                    with torch.no_grad():
                        # actions is [rollout, 7], take first action and add batch dim: [1, 1, 7]
                        action_tensor = torch.from_numpy(actions[:1, :]).unsqueeze(0).to(device).to(z_n.dtype)
                        # Predict next state using predictor
                        # z_n is [1, N_T, D], s_n is [1, 1, 7], action_tensor is [1, 1, 7]
                        with torch.autocast(device_type='cuda', dtype=torch.float16, enabled=False):
                            z_pred = predictor(z_n, action_tensor, s_n)[:, -tokens_per_frame:]
                            # Always normalize (consistent with world_model settings)
                            z_pred = F.layer_norm(z_pred, (z_pred.size(-1),))
                        # Compute predicted energy (L1 distance to goal)
                        if energy_fn_for_logging is not None:
                            predicted_energy = energy_fn_for_logging(z_pred, z_goal).item()
                        else:
                            predicted_energy = torch.mean(torch.abs(z_pred - z_goal)).item()
                        phase3_predicted_energies.append(predicted_energy)
                        print(f" Predicted energy after action: {predicted_energy:.6f}")
                    ...
                    try:
                        ...
                        if teleport_actions:
                            # Teleport arm to target position instantly
                            target_qpos = ik_res.qpos[:ARM_nJnt].copy()
                            sim.data.qpos[:ARM_nJnt] = target_qpos
                            sim.data.qvel[:ARM_nJnt] = np.zeros(ARM_nJnt, dtype=float)
                            sim.data.ctrl[:ARM_nJnt] = target_qpos.copy()
                            sim.forward()
                            current_joint_pos = target_qpos.copy()

                            # Step simulation to allow object physics while arm holds position
                            num_steps = int(round(horizon / step_dt))
                            for _ in range(num_steps):
                                # Force arm to stay exactly at target (true position lock)
                                sim.data.qpos[:ARM_nJnt] = target_qpos
                                sim.data.qvel[:ARM_nJnt] = 0.0
                                sim.data.ctrl[:ARM_nJnt] = target_qpos.copy()
                                sim.advance(render=(render == 'onscreen'))
                                ...
                                if render == 'offscreen' and (frame_counter[0] % steps_per_frame) == 0:
                                    rgb = sim.renderer.render_offscreen(
                                        width=width, height=height,
                                        camera_id=camera_name, device_id=device_id
                                    )
                        ...
                        if not teleport_actions:
                            planned_waypoints = [
                                {'position': current_joint_pos.copy()}
                                for _ in range(int(round(horizon / step_dt)))
                            ]

                    # Execute planned waypoints (only if not teleporting)
                    if not teleport_actions:
                ...
                with torch.no_grad():
                    ...
                    if final_rgb_for_repr is not None:
                        ...
                        # Calculate final distance using configured energy metric
                        if energy_fn_for_logging is not None:
                            final_repr_distance = energy_fn_for_logging(z_n_final, z_goal_final).item()
                        else:
                            final_repr_distance = torch.mean(torch.abs(z_n_final - z_goal_final)).item()
                        phase3_repr_l1_distances.append(final_repr_distance)
                        del h_final, z_n_final, z_goal_final

                if phase3_repr_l1_distances:
                    final_repr_distance = phase3_repr_l1_distances[-1]
                    metric_str = f" (metric: {energy_metric})" if energy_metric != 'l1' else ""
                    print(f"\nFinal representation distance: {final_repr_distance:.6f}{metric_str}")
        ...
        if save_distance_data:
            ...
            all_episode_data['phase3_predicted_energies'].append(
                phase3_predicted_energies if phase3_predicted_energies else [np.nan]
            )
        ...
        if render == 'offscreen' and frames:
            ...
            if skvideo is None:
                ...
                print(
                    ...
                    f"({save_frames.shape[0]} frames, {save_frames.shape[1]}x{save_frames.shape[2]}, {actual_fps} fps)"
                )
    ...
    if save_distance_data:
        ...
        np.savez(
            ...
            phase3_predicted_energies_per_episode=np.array(
                [np.array(d) for d in all_episode_data['phase3_predicted_energies']],
                dtype=object
            ),
