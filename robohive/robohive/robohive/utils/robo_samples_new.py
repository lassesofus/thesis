...
"""Franka reach with IK + min-jerk and optional V-JEPA planning."""
...
try:
    ...
    warnings.filterwarnings(
        "ignore",
        message=(
            r".*The 'repr' attribute with value False was provided to the "
            r"`Field\(\)` function.*"
        ),
    ...
    warnings.filterwarnings(
        "ignore",
        message=(
            r".*The 'frozen' attribute with value True was provided to the "
            r"`Field\(\)` function.*"
        ),
        category=UserWarning,
    )
...
# Ensure V-JEPA repo is importable when running locally
...
try:
    import skvideo.io  # type: ignore[import]
except Exception:
    skvideo = None  # type: ignore[assignment]
...
def main(sim_path, horizon, radius, render, camera_name, out_dir, out_name,
    ...
    while True:
        ...
        while targets_reached < num_targets:
            ...
            # Measure distances during approach (NEW)
            # Re-execute to track distances (lightweight)
            sim.data.qpos[:ARM_nJnt] = ARM_JNT0 if targets_reached == 1 else current_joint_pos
            for wp in approach_waypoints[::max(1, len(approach_waypoints)//20)]:  # Sample every ~5%
                sim.data.ctrl[:ARM_nJnt] = wp['position']
                sim.forward()
                ee_pos = sim.data.site_xpos[ee_sid].copy()
                dist = np.linalg.norm(ee_pos - target_pos)
                target_distances_this_approach.append(dist)
